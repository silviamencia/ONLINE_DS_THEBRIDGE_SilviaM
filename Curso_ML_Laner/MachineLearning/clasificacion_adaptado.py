...
os.chdir(r"C:\Users\LENOVO\Documents\DataVM\CursoPythonCompleto\17-Modelos\Modelos_Clasificacion")
...
prediccion_rl_nor = RegLog.predict(test_data_X)

# Calculamos l¡el nivel de acierto en test

from sklearn.metrics import confusion_matrix

confusion_matrix(test_data_y, prediccion_rl_nor)
...
X_train, X_test, y_train, y_test = train_test_split(datosfinal, y, test_size=0.25, random_state=44)
...
prediccion_knn = knn.predict(test_data_X)

# Calculamos l¡el nivel de acierto en train y en test

from sklearn.metrics import confusion_matrix

confusion_matrix(test_data_y, prediccion_knn)
...
accuracy_score(test_data_y, prediccion_knn)
...
metrics.precision_score(prediccion_knn, test_data_y) #NO
metrics.recall_score(test_data_y, prediccion_knn)
metrics.precision_score(test_data_y,prediccion_knn)
...
X_train, X_test, y_train, y_test = train_test_split(datosfinalNorm, y, test_size=0.25, random_state=44)
...
# Realizamos la prediccion sobre los datos de test
test_data_X = np.array(test_data_X)
prediccion_knn_nor = knn.predict(test_data_X)

# Calculamos l¡el nivel de acierto en train y en test

from sklearn.metrics import confusion_matrix

confusion_matrix(test_data_y, prediccion_knn_nor)
...
# Accuracy muestra el porcentaje de aciertos del modelo
from sklearn.metrics import accuracy_score
accuracy_score(test_data_y, prediccion_knn_nor)

# la precision mide el porcentaje de aciertos que tiene el modelo cuando predice 0 (no se fuga)
# La sensitividad (recall) midel el porcentaje de acierto cuando predice 1 (fuga)

from sklearn import metrics
metrics.precision_score(test_data_y, prediccion_knn_nor)
metrics.recall_score(test_data_y, prediccion_knn_nor)
...
cross_val_score(Bayes, X, y, cv=10).mean()
cross_val_score(Bayes, X, y, cv=10, scoring='precision').mean()
cross_val_score(Bayes, X, y, cv=10, scoring='recall').mean()
'''
#########################################   CAMBIO
y=datosfinalNorm['Churn']
x=datosfinalNorm.drop(['Churn'], axis = 1)
...
predicciones_nb = gnb.predict(X_test)
accuracy_score(y_test, predicciones_nb)
metrics.recall_score(y_test, predicciones_nb)
...
cross_val_score(gnb, x, y, cv=10, scoring='precision').mean()
cross_val_score(gnb, x, y, cv=10, scoring='recall').mean()
##########################################   FIN 
...
modelo_dtc=tree.DecisionTreeClassifier(class_weight=None, criterion='gini', max_depth=8,max_features=None, max_leaf_nodes=None, min_impurity_decrease=0.0,min_samples_leaf=1, min_samples_split=2, min_weight_fraction_leaf=0.0, random_state=None, splitter='best')
...
'''
#########################################   CAMBIO
arbol = modelo_dtc.fit(X_train, y_train)

predicciones_dtc = arbol.predict(X_test)
accuracy_score(y_test, predicciones_dtc)

importanciasarbol=pd.DataFrame(arbol.feature_importances_)
importanciasarbol
importanciasarbol.index=(X_train.columns)
importanciasarbol

cross_val_score(modelo_dtc, x, y, cv=10).mean()
cross_val_score(modelo_dtc, x, y, cv=10, scoring='precision')
cross_val_score(modelo_dtc, x, y, cv=10, scoring='recall')
...
from sklearn import tree
#tree.plot_tree(arbol, feature_names = X.columns.tolist(), class_names=['0','1'], filled=True) 
tree.plot_tree(arbol, feature_names = x.columns.tolist(), class_names=['0','1'], filled=True) 
...
modelo_rf=RandomForestClassifier(bootstrap=True, class_weight=None, criterion='gini',max_depth=None, max_features='sqrt', max_leaf_nodes=None,min_impurity_decrease=0.0,min_samples_leaf=1, min_samples_split=2,min_weight_fraction_leaf=0.0, n_estimators=1000, n_jobs=-1, oob_score=False, random_state=None, verbose=0,warm_start=False)
...
cross_val_score(modelo, X, y, cv=10, scoring='precision')
cross_val_score(modelo, X, y, cv=10, scoring='recall')
...
RF = modelo_rf.fit(X_train, y_train)

predicciones_rf = RF.predict(X_test)
accuracy_score(y_test, predicciones_rf)
metrics.recall_score(y_test, predicciones_rf)

importancias=pd.DataFrame(RF.feature_importances_)
importancias.index=(x.columns)
importancias

cross_val_score(modelo_rf, x, y, cv=10).mean()
cross_val_score(modelo_rf, x, y, cv=10, scoring='precision').mean()
cross_val_score(modelo_rf, x, y, cv=10, scoring='recall').mean()
...
modelo_xgb = xgb.XGBClassifier(base_score=0.5, colsample_bylevel=1, colsample_bytree=1, gamma=0, learning_rate=0.1, max_delta_step=0, max_depth=6,min_child_weight=1,missing=1,n_estimators=1000,objective='binary:logistic',reg_alpha=0,reg_lambda=1,scale_pos_weight=1, seed=0, subsample=1)
...
XGBoost=modelo_xgb.fit(X_train, y_train)
predicciones_xgb = XGBoost.predict(X_test)
accuracy_score(y_test, predicciones_xgb)
...
cross_val_score(modelo_xgb, x, y, cv=10)
cross_val_score(modelo_xgb, x, y, cv=10, scoring='precision')
cross_val_score(modelo_xgb, x, y, cv=10, scoring='recall')
